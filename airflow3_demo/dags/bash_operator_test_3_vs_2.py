"""
Airflow 3 BashOperator Tracer - Compatible with Airflow 3.x
This script traces BashOperator execution in Airflow 3 for comparison with Airflow 3.
...
def trace_bashoperator_execution():
    """
    Execute a BashOperator task and trace all function calls - Airflow 3 version
    """
    typewriter_print(f"\n{Colors.OKCYAN}🔧 Initializing Airflow 3 components...{Colors.ENDC}")
    animate_progress_bar("Loading Airflow 3 modules", duration=0.8)

    # Import Airflow 3 components BEFORE starting trace
    # Note: Airflow 3 uses different import paths
    ...
    typewriter_print(f"{Colors.OKGREEN}✓ Airflow 3 imports complete{Colors.ENDC}\n")
    ...
    dag = DAG(
        ...
        schedule=None,  # Note: Airflow 3.10+ uses 'schedule' like Airflow 3
    )

    bash_task = BashOperator(
        task_id='test_bash_task',
        bash_command='echo "Hello from Airflow 3 BashOperator"',
        dag=dag,
    )
    ...
    print_banner("🔍 LIVE EXECUTION TRACE - Airflow 3", '═')
    typewriter_print(f"{Colors.DIM}Watching Airflow 3 internals in real-time...{Colors.ENDC}\n")
    ...
    try:
        # Execute the task - Airflow 3 style
        # Note: Airflow 3 uses different context structure
...
def print_summary():
    ...
    print_banner("📊 EXECUTION SUMMARY - Airflow 3", '═')
...
if __name__ == "__main__":
    ...
    # Title screen
    print_banner("🚀 Airflow 3 BASHOPERATOR INTERNALS TRACER 🚀", '═')

    info_lines = [
        f"{Colors.DIM}Airflow Version:{Colors.ENDC}         3.x",
    ...
    try:
        ...
        completion_lines = [
            ...
            f"{Colors.OKGREEN}✓{Colors.ENDC} {len(airflow_modules)} Airflow 3 modules analyzed",
